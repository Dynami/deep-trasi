...
def get_options(conn, df_index_filter, option_step=250.0, upper_bound_strike=30_000, lower_bound_strike=10_000, maturity_filter=1.):
    ...
    _max = 0
    for d in np.unique(df_index_filter.index):
        # print(d)
    ...
    df_options_sparse['price_distance_norm'] = abs(df_options_sparse.strike - df_index_filter.close) / df_options_sparse.strike
    df_options_sparse['price_distance_sign'] = np.where(df_options_sparse.strike - df_index_filter.close > 0, 1, 0)
    ...
    for date in np.unique(df_index_filter.index):
    ...
    return pd.DataFrame(index=df_index_filter.index, data=option_data, columns=option_labels);


def get_prices(conn, look_fwd, threshold):
    ...
    df_index_norm['sparse_target'] = np.where(df_index_norm['target'] > threshold, 1,
                                               np.where(df_index_norm['target'] < -threshold, -1, 0))

    df_target_norm = df_index_norm[['target', 'sparse_target']]
...
def get_futures(conn, df_index_filter):
    ...
    future_labels = np.array([['f_' + l + '_' + str(i) for l in df_future.columns.values] for i in range(1, 5)], dtype=str)
...
def main(conn, look_fwd=10, threshold=0.05, option_step=250.0):
    ...
    df = pd.concat([df_prices_norm, df_futures, df_option_data, df_target_norm], axis=1)
