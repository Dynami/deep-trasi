...
def main(df, split_ratio=0.80, models=None):
    ...
    if (False):
        ...
        clf = GridSearchCV(model, parameters, cv=4, scoring='precision')
        ...
        from sklearn.svm import SVC
        model = SVC(class_weight=class_weight, probability=True, kernel='linear', C=0.1, gamma=.1)
    ...
    print('Std   >>>>> %.3f' % nz_std)
    print('Samples>>>> %s' % x_test_data.shape[0])
    pass
